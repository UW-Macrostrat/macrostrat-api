  ...
  async.parallel(
    {
      places: (callback) => {
        https.get(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${req.query.query}.json?access_token=pk.eyJ1IjoiamN6YXBsZXdza2kiLCJhIjoiY2pjMjBiYWRjMDh2ZzJ4cHIwMjdyeWpieCJ9.EO2U9fSUuSPFvJ8LBQ4QSg&types=country,region,locality,place,poi.landmark`,
          (res) => {
                ...
                callback(
                  ...
                  body.features.map((place) => {
                    return {
                      ...
                      category: "place",
                      place_type:
                        place.place_type && place.place_type.length
                          ? place.place_type[0]
                          : "",
      ...
      macrostrat: (callback) => {
        ...
        LIMIT 100
      `,
          [`${req.query.query}%`, foundCategories],
          (error, result) => {
            if (error) {
