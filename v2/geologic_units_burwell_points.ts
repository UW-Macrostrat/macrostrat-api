...
module.exports = function(req, res, next) {
  ...
  if (req.query.minlat && req.query.minlng && req.query.maxlat && req.query.maxlng) {
    where.push('ST_Intersects(geom, ST_Envelope(ST_GeomFromText($' + (where.length + 1) + ')))')
    params.push('SRID=4326;LINESTRING(' + req.query.minlng + ' ' + req.query.minlat + ',' + req.query.maxlng + ' ' + req.query.maxlat + ')')
  }

  if (!where.length && !limit.length) {
    return larkin.error(req, res, next, 'No valid query parameters passed', 400)
  ...
  `, params, function(error, result) {
    ...
    if (req.query.format && req.query.format === 'csv') {
      ...
      dbgeo.parse(result.rows, {
        outputFormat: (req.query.format && api.acceptedFormats.geo[req.query.format]) ? larkin.getOutputFormat(req.query.format) : 'geojson',
        geometryType: 'll',
        geometryColumn: ['longitude', 'latitude'],
        precision: 6
      }, function(error, output) {
        if (error) {
          return larkin.error(req, res, next, 'An error occurred while parsing the output', 500)
        }
        larkin.sendData(req, res, next, {
          format: 'json',
