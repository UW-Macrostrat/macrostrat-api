...
module.exports = function(req, res, next, cb) {
  if (Object.keys(req.query).length < 1) {
    ...
    async.parallel([
      ...
      function(callback) {
        if (req.query.carto) {
          if (req.query.carto == 'large') carto_scale = ' AND m.legend_id IN (SELECT distinct legend_id from maps.map_legend JOIN carto_new.large USING (map_id))';
          if (req.query.carto == 'medium') carto_scale = ' AND m.legend_id IN (SELECT distinct legend_id from maps.map_legend JOIN carto_new.medium USING (map_id))';
          if (req.query.carto == 'small') carto_scale = ' AND m.legend_id IN (SELECT distinct legend_id from maps.map_legend JOIN carto_new.small USING (map_id))';
          if (req.query.carto == 'tiny') carto_scale = ' AND m.legend_id IN (SELECT distinct legend_id from maps.map_legend JOIN carto_new.tiny USING (map_id))';
        }
      ...
      if (where.length < 1 && where_lith.length < 1 && !("sample" in req.query) && !req.query.carto) {
