...
*/
  ...
  larkin.sendData = function (req, res, next, options, outgoing) {
    if (!options.format) {
      options.format = api.acceptedFormats.standard[req.query.format]
        ? req.query.format
        : "json";
    }
    if (!options.bare) {
  ...
  larkin.defineRoute = function (route, callback) {
    this.defineFields(route, function (fields) {
      ...
      if (defs[route].options.response_types) {
        routeDefinition.options.response_types =
          defs[route].options.response_types;
      }
  ...
  larkin.getOutputFormat = function (requestedFormat) {
    switch (requestedFormat) {
      ...
      case "topojson_bare":
        return "topojson";
      default:
        return requestedFormat === "geojson" || requestedFormat === "topojson"
          ? requestedFormat
          : "geojson";
    }
  };
  ...
  larkin.verifyKey = function (key, callback) {
    if (!validator.isUUID(key)) {
      ...
      this.queryPg(
        "geomacro",
        "SELECT key, admin FROM apikeys WHERE key = $1",
        [key],
        function (error, data) {
          if (error) {
  ...
  larkin.summarizeAttribute = function (data, type) {
    ...
    var cats = _.groupBy(mommaCat, function (d) {
        return d[type + "_id"];
      }),
      total_cats = Object.keys(cats)
        .map(function (cat) {
          return cats[cat].length;
        })
        .reduce(function (a, b) {
          return a + b;
        }, 0),
  ...
  larkin.getRefs = function (key, data, callback) {
    ...
    if (data.type && data.type === "Topology") {
      ...
      data = data.features.map(function (d) {
        return d.properties;
      });
    }
      ...
      larkin.queryPg(
        ...
        function (error, result) {
          ...
          for (var i = 0; i < result.rows.length; i++) {
            refs[result.rows[i]["source_id"]] =
              larkin.normalizeRefField(result.rows[i].authors) +
              larkin.normalizeRefField(result.rows[i].ref_title) +
              larkin.normalizeRefField(result.rows[i].isbn_doi) +
