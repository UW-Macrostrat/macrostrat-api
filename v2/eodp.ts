    ...
    if (error) {
      ...
      if (req.query.format !== "csv") {
        for (var i = 0; i < response.length; i++) {
          response[i].top_depth = larkin.jsonifyPipes(
            response[i].top_depth,
            "floats",
          );
          response[i].bottom_depth = larkin.jsonifyPipes(
            response[i].bottom_depth,
            "floats",
          );
          response[i].primary_lith = larkin.jsonifyPipes(
            response[i].primary_lith,
            "strings",
          );
          response[i].lith_id = larkin.jsonifyPipes(
            response[i].lith_id,
            "integers",
          );
          response[i].minor_lith = larkin.jsonifyPipes(
            response[i].minor_lith,
            "strings",
          );
        }
      }
