...
exports.root = function(req, res, next) {
  res.send("This is the root of the macrostrat API");
}

/*    /api/columns?id=17    */
exports.columns = function(req, res, next) {
  if (req.query.id) {
    async.series({
      query1: function(callback) {
        api.intermediateQuery("SELECT id, col_name FROM cols WHERE id = ?", [req.query.id], function(result) {
          if (result.length === 0) {
            api.error(res, next, "No results found.");
          } else {
      ...
      query2: function(callback) {
        api.intermediateQuery("SELECT units.id AS unit_id, units_sections.section_id, strat_name, max_thick,\
    ...
    function(err, results) {
      ...
      api.sendData(column, res, next);
    });

  } else {
    api.error(res, next, "Please provide a parameter id. Example: /api/columns?id=17");
  }
}

/*    /api/fossils?interval_name=Permian || /api/fossils?age=250 || /api/fossils?age_top=100&age_bottom=200    */
exports.fossils = function(req, res, next) {
  async.waterfall([
    function(callback) {
      if (req.query.interval_name) {
        api.intermediateQuery("SELECT age_bottom,age_top,interval_name from intervals where interval_name = ? limit 1", [req.query.interval_name], function(result) {
          if (result.length === 0) {
            api.error(res, next, "No results found");
          } else {
        ...
        api.error(res, next, "Please provide an interval_name, age, or age_top & age_bottom.<br> Examples: <br>/api/fossils?interval_name=Permian <br> /api/fossils?age=271 <br> /api/fossils?age_top=200&age_bottom=250");
      }
    },
    function(data, callback) {
      api.intermediateQuery("SELECT COUNT(pbdb.occurrences.occurrence_no) AS occ, pbdb_matches.collection_no,\
    ...
    dbgeo.parse({
      ...
      "callback": function(error, result) {
        if (error) {
          api.error(res, next, error);
        } else {
          api.sendData(result, res, next);
        }
      }
    });
...
exports.strats = function(req, res, next) {
  async.waterfall([
    function(callback) {
      if (req.query.interval_name) {
        api.intermediateQuery("SELECT age_bottom,age_top,interval_name from intervals where interval_name = ? LIMIT 1", [req.query.interval_name], function(result) {
