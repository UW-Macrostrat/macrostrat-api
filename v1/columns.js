...
module.exports = function(req, res, next) {
  ...
  async.waterfall([
    ...
    function(data, callback) {
      ...
      if (data.col_ids) {
        where += " AND units_sections.col_id IN (?)";
      ...
      var additionalFields = (req.query.response === "long") ? "col_name, col_group, col_groups.id AS col_group_id, max(lookup_unit_intervals.b_age) AS b_age, min(lookup_unit_intervals.t_age) AS t_age, GROUP_CONCAT(DISTINCT units_sections.section_id SEPARATOR '|') AS sections, sum(pbdb.colls) AS pbdb_collections, sum(pbdb.occs) AS pbdb_occs, " : "";

      var additionalJoins = (req.query.response === "long") ? " LEFT JOIN (select sum(pbdb_matches.occs) AS occs, count(distinct collection_no) AS colls, pbdb_matches.unit_id FROM pbdb_matches GROUP BY unit_id) pbdb ON pbdb.unit_id = units.id " : ""

      larkin.query("SELECT " + geo + " col_areas.col_id, " + additionalFields + "round(cols.col_area, 1) AS area, GROUP_CONCAT(DISTINCT units.id SEPARATOR '|') AS units, sum(max_thick) max_thick, sum(min_thick) min_thick, sum(max_thick) AS lith_max_thick, sum(min_thick) AS lith_min_thick, LT2.lts lith_types \
        FROM col_areas \
        JOIN cols ON cols.id = col_areas.col_id \
        JOIN units_sections ON units_sections.col_id = cols.id \
        JOIN units ON units.id = units_sections.unit_id \
        ...
        JOIN (SELECT unit_id, round(sum(comp_prop*max_thick), 1) cpm, round(sum(comp_prop*min_thick), 1) cpl FROM unit_liths JOIN liths on lith_id=liths.id JOIN units on unit_id=units.id WHERE ?? like ? GROUP BY unit_id) LT ON LT.unit_id= units_sections.unit_id \
        " + stratJoin + " \
        JOIN (SELECT col_id, GROUP_CONCAT(distinct lith_type SEPARATOR '|') lts FROM liths JOIN unit_liths on lith_id=liths.id JOIN units_sections ON units_sections.unit_id = unit_liths.unit_id JOIN lookup_unit_intervals ON lookup_unit_intervals.unit_id = units_sections.unit_id WHERE ?? like ? " + first_where + " GROUP BY col_id) LT2 on LT2.col_id=col_areas.col_id \
