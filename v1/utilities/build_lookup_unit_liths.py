...
cursor.execute("INSERT INTO lookup_unit_liths (unit_id,lith_class, lith_type, lith_short) SELECT unit_id, GROUP_CONCAT(distinct lith_class  SEPARATOR '|'), GROUP_CONCAT(distinct lith_type  SEPARATOR '|'), GROUP_CONCAT(lith,' ',comp_prop SEPARATOR '|') FROM unit_liths JOIN liths on lith_id=liths.id GROUP BY unit_id")
